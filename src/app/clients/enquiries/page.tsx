...
export default function ClientEnquiriesPage() {
  ...
  const fetchEnquiries = async () => {
    try {
      const res = await fetch("/api/clients/enquiry");
      const data = await res.json();
      if (res.ok) {
        setEnquiries(data.enquiries);
        if (data.enquiries && data.enquiries.length > 0) {
          const client = data.enquiries[0];
          // Pre-populate fields if they are currently blank
          setName(prev => prev || client.name || "");
          setEmail(prev => prev || client.email || "");
          setMobile(prev => prev || client.mobile || "");
        }
      } else {
        throw new Error(data.error);
      }
    } catch (err) {
      console.error("Error fetching client enquiry details:", err);
    }
  };
  ...
  const renderStatusBadge = (statusVal: string) => {
    switch (statusVal) {
      ...
      case "active":
        return <Badge variant="outline" className="bg-emerald-500/15 text-emerald-400 border-emerald-500/30 font-medium">Active Client</Badge>;
      case "inactive":
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 relative z-10">
        <div>
          <h2 className="text-3xl font-display font-bold tracking-tight text-foreground flex items-center gap-2">
            My Enquiries & Support
          </h2>
          <p className="text-sm text-muted-foreground mt-1">
            Submit subscription requests, report issues, and check admin responses.
          </p>
        </div>
        <Button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 relative z-10 max-w-6xl">
          ...
          <div className="bg-card/30 backdrop-blur-xl border border-border/10 rounded-3xl p-6 shadow-elegant space-y-5">
            <div>
              <h3 className="text-sm font-bold text-foreground flex items-center gap-1.5">
                <MessageSquare className="h-4 w-4 text-brand" /> Submit New Enquiry
              </h3>
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="bg-card/30 backdrop-blur-xl border border-border/10 rounded-3xl p-6 shadow-elegant space-y-5">
            <div>
              <h3 className="text-sm font-bold text-foreground flex items-center gap-1.5">
                <Clock className="h-4 w-4 text-purple-400" /> My Enquiry History
              </h3>
            ...
            ) : enquiries.length === 0 || !enquiries[0].subscription ? (
